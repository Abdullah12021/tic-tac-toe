...
char selectOpponent() {
    ...
    while (choice != '1' && choice != '2') {
        cout << "\033[2J\033[H\033[?25l"; // Clear screen
        cout << AMBER << "=========================================\n";
        cout << BOLD << CREAM << "            SELECT GAME MODE             \n";
        cout << AMBER << "=========================================\n\n";

        cout << CREAM << "  [1] Player vs Computer \n";
        cout << "  [2] Player vs Player \n\n";
        cout << BOLD << SAGE << "  >>> Press 1 or 2 to select: " << RESET;
...
int main(){
    ...
    while (input != 'q' && winner == ' ' && moveCount < 9) {
        ...
        if (turn == 'X' || gameMode == '2') {
            ...
            // Loop keeps running until it finds a board slot that is actually empty (' ')
            do {
                computerMove = rand() % 9;
            } while (board[computerMove] != ' ');

            // Place the mark instantly at that random empty slot
            placeMark(computerMove, board, turn);
            moveCount++;
            winner = checkWin(board);
        }

        // Redraw screen after either player moves
        printBoard(cursor, board, turn);
